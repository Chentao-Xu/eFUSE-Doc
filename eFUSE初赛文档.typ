...
#show: project.with(
  ...
  abstract_zh: [
  eFuse是一个针对FUSE性能优化设计的扩展方案，通过结合eBPF技术，尝试从多个方向切入，解决FUSE一直以来饱受诟病的性能问题，同时保留FUSE开发便捷等、安全性高等优点。本项目旨在不改变FUSE整体架构和接口标准的前提下，通过eBPF技术实现对FUSE性能的优化，同时注重与现有FUSE文件系统的兼容性、安全性、可扩展性和易用性，实现一个对用户友好的FUSE优化方案。

  针对FUSE在高频负载场景下的性能瓶颈和上述目标，eFuse对FUSE文件系统请求的处理路径、调度管理等方面均进行优化，用以解决由于频繁的用户态/内核态切换、高频负载下严重的锁争用等要素而导致的性能问题。

  为此，我们设计五大技术目标模块以及目前完成情况如下：
