...
λ-IO 是一套跨主机与存储设备的统一 I/O 堆栈，旨在打破传统 I/O 仅支持数据存取的局限，将计算能力引入 I/O 路径，使得应用程序能够在主机与设备两端透明地执行计算逻辑。#bib_cite(<285746>) 对于数据库等数据密集型应用，以往需要将大规模数据从存储设备搬运到主机内存再处理，带来巨大的数据移动开销。λ-IO 通过在操作系统 I/O 堆栈中集成可编程的计算运行时，减少数据搬移，并充分利用主机与设备双方的计算潜力。

λ-IO 的核心贡献体现在三个方面。首先，它提供了统一的接口，在传统文件读写的基础上，允许应用加载并调用自定义的 λ 函数，从而在数据流经 I/O 堆栈时触发计算。其次，它扩展 eBPF 提出了 sBPF 运行时，放宽了对指针访问和循环的限制，支持更复杂的存储计算逻辑，同时保持字节码验证与运行时安全。最后，它设计了动态调度机制，通过对数据规模、缓存命中率、存储带宽、主机与设备的计算带宽等因素建模，预测请求在主机或设备端的执行时间，并将任务分配到更优的一侧执行。

在运行时层面，λ-IO 同时在内核与设备端部署。内核运行时位于 VFS 和页面缓存之上，利用 mmap 式映射减少数据拷贝开销；设备运行时依赖下推的文件元数据直接定位文件块，避免额外的数据传输。由于数据可能在用户空间、内核缓存和设备端同时存在，λ-IO 通过文件锁、缓存刷新和一致性协议来保证主机与设备之间的状态同步。

整体来看，λ-IO 将 I/O 堆栈从单纯的数据传输通道扩展为可编程的计算平台，兼容现有 POSIX 文件接口，复用操作系统 I/O 堆栈中的安全与资源管理机制，同时提供高效的跨端协同调度能力。这为计算存储的落地提供了统一、灵活且高效的系统支持。
